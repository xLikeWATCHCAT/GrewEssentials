...
public class MessageFile extends YamlConfiguration {
    ...
    private MessageFile(InputStream stream)
    {
        loger = Bukkit.getLogger();
        init(stream);
    }

    public MessageFile(Plugin plugin) { this(plugin, "message.yml"); }

    private MessageFile(Plugin plugin, File file)
    {
        Validate.notNull(file, "File cannot be null");
    ...
    private MessageFile(Plugin plugin, String filename) { this(plugin, new File(plugin.getDataFolder(), filename)); }
    ...
    private void check(File file)
    {
        ...
        try
        {
            ...
            {
                MessageFile newcfg = new MessageFile(stream);
                MessageFile oldcfg = new MessageFile(file);
                ...
                if ((newver != null) && !(newver.equalsIgnoreCase(oldver)))
                {
                    ...
                    try {
                        ...
                        oldcfg.save(new File(file.getParent(), "Backup"+File.separator +"message"+File.separator+"backup."+filename+"."+time));
