...
public class logMessageFile extends YamlConfiguration {
    private File file;
    ...
    private logMessageFile(File file)
    {
    ...
    private logMessageFile(InputStream stream)
    {
        loger = Bukkit.getLogger();
        init(stream);
    }

    public logMessageFile(Plugin plugin) { this(plugin, "logMessage.yml"); }

    private logMessageFile(Plugin plugin, File file)
    {
        Validate.notNull(file, "file cannot be null");
    ...
    private logMessageFile(Plugin plugin, String filename) { this(plugin, new File(plugin.getDataFolder(), filename)); }
    ...
    private void check(File file)
    {
        ...
        try
        {
            ...
            {
                logMessageFile newcfg = new logMessageFile(stream);
                logMessageFile oldcfg = new logMessageFile(file);
                ...
                if ((newver != null) && !(newver.equalsIgnoreCase(oldver)))
                {
                    ...
                    try {
                        String time = StringUtils.getNanon();
                        oldcfg.save(new File(file.getParent(), "Backup"+File.separator +"logMessage"+File.separator+"backup."+filename+"."+time));
                       // loger.warning("配置文件: " + filename + " 已备份为 backup."+filename+"."+time);
                    } catch (IOException e) {
    ...
    public void reload() throws IOException { backup(); init(file); }
