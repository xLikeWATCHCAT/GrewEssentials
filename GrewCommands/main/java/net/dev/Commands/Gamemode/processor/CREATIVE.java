    ...
    public static void setPlayerGamemodeToCREATIVE(Player p,CommandSender b){
        if (p.getGameMode() == GameMode.CREATIVE) {
            b.sendMessage(StringUtils.translateColorCodes(GrewEssentials.getInstance().Message.getString("Gamemode.Presence_Message"))
                    ...
                    .replace("$type",StringUtils.translateColorCodes(GrewEssentials.getInstance().Message.getString("Gamemode.Creative.Type"))));
            return;
        }
        p.setGameMode(GameMode.CREATIVE);
        b.sendMessage(StringUtils.translateColorCodes(GrewEssentials.getInstance().Message.getString("Gamemode.Message")).replace("$prefix", StringUtils.Prefix).replace("$playername",p.getName()).replace("$type",StringUtils.translateColorCodes(GrewEssentials.getInstance().Message.getString("Gamemode.Creative.Type"))));
