...
public class SPECTATOR{
    ...
    public static void setPlayerGamemodeToSPECTATOR(Player p,CommandSender b){
        ...
        if(b instanceof Player){
            ...
            LogUtils.writeLog(StringUtils.removeColorCodes(GrewEssentials.getInstance().log.getString("Gamemode").replace("$player",p.getName()).replace("$type",StringUtils.removeColorCodes(GrewEssentials.getInstance().Message.getString("Gamemode.Spectator.Type"))).replace("$doplayer",Console).replace("$playeruuid",p.getUniqueId().toString()).replace("$playerip",p.getAddress().getHostName()).replace("$playerisop",String.valueOf(p.isOp())).replace("$doplayeruuid",Console).replace("$doplayerip",Console).replace("$doplayerisop","true")));
        }
    }
